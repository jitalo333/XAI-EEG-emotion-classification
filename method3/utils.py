...
def heatmap_plot(args, heatmap_data, save_path, pred_class, bands_labels):
    ...
    sns.heatmap(
        ...
        cmap='turbo',
        cbar_kws={'label': 'Attribution (Integrated Gradients)'},
        vmin=0, vmax=1
    )
    plt.title(f"Integrated Gradients - {args['model_type']}\nSubject {args['subject_id']} | Sample {args['sample_idx']} | Class {pred_class}")
    plt.xlabel("Frequency Bands")
    plt.ylabel("EEG Channels")
    plt.tight_layout()
    save_plot(save_path, f"ig_heatmap_sub{args['subject_id']}_idx{args['sample_idx']}_{args['model_type']}.png")
...
def channel_importance_plot(args, heatmap_data, save_path, pred_class):
    ...
    plt.title(f"Channel Importance (IG) - {args['model_type']}\nSubject {args['subject_id']} | Class {pred_class}")
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    save_plot(save_path, f"ig_channels_sub{args['subject_id']}_idx{args['sample_idx']}_{args['model_type']}.png")
...
def band_importance_plot(args, heatmap_data, save_path, pred_class, bands_labels):
    ...
    plt.ylabel("Mean Importance")
    plt.title(f"Band Importance (IG) - {args['model_type']}\nSubject {args['subject_id']} | Class {pred_class}")
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
    save_plot(save_path, f"ig_bands_sub{args['subject_id']}_idx{args['sample_idx']}_{args['model_type']}.png")