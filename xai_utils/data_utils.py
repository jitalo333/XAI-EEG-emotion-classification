...
def heatmap_plot(args, heatmap_data, save_path, pred_class, bands_labels):
    ...
    # Título dinámico
    plt.title(f"{method_name} Map - {args['model_type']}\nSubject {args['subject_id']} | {title_context} | Class {pred_class}")
...
def channel_importance_plot(args, heatmap_data, save_path, pred_class):
    ...
    plt.title(f"Channel Importance ({method_name}) - {args['model_type']}\nSubject {args['subject_id']} | {title_context} | Class {pred_class}")
...
def band_importance_plot(args, heatmap_data, save_path, pred_class, bands_labels):
    ...
    plt.title(f"Band Importance ({method_name}) - {args['model_type']}\nSubject {args['subject_id']} | {title_context} | Class {pred_class}")
    plt.grid(axis='y', alpha=0.3)
    plt.tight_layout()
